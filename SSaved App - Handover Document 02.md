    ...
    <script>
        ...
        function loadDataFromDB() {
            ...
            tx.objectStore("cards").getAll().onsuccess = (e) => {
                const raw = e.target.result.map(c => ({...c, imageUrl: URL.createObjectURL(c.file), folderId: c.folderId || INBOX_ID }));
        ...
        function deleteFolder(id) {
            ...
            if(!confirm("Delete this folder? Cards inside will be moved to Inbox.")) return;
            
            const cardsToMove = appData.cards.filter(c => c.folderId === id);
            cardsToMove.forEach(c => { c.folderId = INBOX_ID; saveCardToDB(c); });
